...
export class TreeNodeView extends React.Component<{
	...
	private readonly _autorun = autorun(() => {
		if (this.props.model.isMarked && this.rootDiv) {
			this.rootDiv.scrollIntoView({ block: "nearest" });
		}
	});
...
function renderItems(items: Item[]): React.ReactNode {
	return items.map((i, idx) => {
		switch (i.emphasis) {
			case "style1":
				return (
					<span key={idx} className="style1">
						{i.text}
					</span>
				);
			case "style2":
				return (
					<span key={idx} className="style2">
						{i.text}
					</span>
				);
			case "style3":
				return (
					<span key={idx} className="style3">
						{i.text}
					</span>
				);
			default:
				return (
					<span key={idx} className="unstyled">
						{i.text}
					</span>
				);
		}
